...
def plot_temp_ensemble(
    ...
    for agg_type in agg_types:
        ...
        y_max = np.max(all_mse_full) * 1.05
...
def plot_ensemble(
    ...
    if 'baseline' in data and agg_type in data['baseline']:
        baseline_y_val = data['baseline'][agg_type][0]
    ...
    for disturb_type, disturb_data in sorted_disturbs:
        ...
        line, = ax.plot(
            ...
            linewidth=linewidth,
            linestyle=linestyle,
            label=disturb_type.replace("_", " ").title(), 
            alpha=0.95,
            zorder=zorder_val
        )
        if is_legend_plot:
            legend_handles.append(line)
            legend_labels.append(disturb_type.replace("_", " ").title())
    ...
    if global_y_lim:
        ax.set_ylim(global_y_lim[0], global_y_lim[1])
...
def plot_len_ensemble(
    ...
    cmap = LinearSegmentedColormap.from_list('custom_gradient', CUSTOM_COLORS, N=512)

    first_disturb = next(iter(data.values()))
    num_samples_original = len(first_disturb[agg_types[0]])
    max_pow = int(np.floor(np.log2(num_samples_original)))
    x_tick_pows = [2**k for k in range(max_pow + 1)]
    ...
    for disturb_type in data.keys():
        try:
            L_val = int(disturb_type.split("l")[-1].split("_")[0])
            L_list.append(L_val)
        except (IndexError, ValueError):
            continue
    
    if not L_list:
        L_min, L_max = 512, 1024 
        print("Warning: Could not parse Context Length (L) values. Using default range for color mapping.")
    else:
        L_min = min(L_list)
        L_max = max(L_list)
    norm = Normalize(vmin=np.log2(L_min)-1, vmax=np.log2(L_max))
    log_min = np.floor(norm.vmin)  
    log_max = np.ceil(norm.vmax)   
    ticks_log = np.arange(log_min, log_max + 1, 1)  
    ticks_original = [int(2 ** t) for t in ticks_log]  
    ...
    for agg_type in agg_types:
        ...
        sorted_disturbs = sorted(
            data.items(),
            key=lambda x: int(x[0].split("l")[-1].split("_")[0]) 
        )
        ...
        for idx, (disturb_type, disturb_data) in enumerate(sorted_disturbs):
            ...
            try:
                L_val = int(disturb_type.split("l")[-1].split("_")[0])
                label_name = f"L={L_val}"
            except:
                L_val = L_min 
                label_name = disturb_type.replace("_", " ").title()

            line_color = cmap(norm(np.log2(L_val)))
            linewidth = 2.0 
            linestyle = "-"
        ...
        ax.set_xticks(x_tick_pows)
        ax.set_xticklabels([f"$2^{k}$" for k in range(len(x_tick_pows))], fontsize=13) 
        all_mse_full = [mse for dt in data.values() for mse in dt.get(agg_type, [])]
        y_min = np.min(all_mse_full) -0.1
        y_max = np.max(all_mse_full)
        ax.set_ylim(y_min, y_max)
        ax.tick_params(axis='y', labelsize=13)
        ax.grid(True, axis='y', alpha=0.3, linestyle='-.')
        ax.grid(False, axis='x')
        ...
        cbar.ax.tick_params(labelsize=10)
        cbar.set_label("Context Length (L)", fontsize=11, fontweight='bold',rotation=270,labelpad=15)
        cbar.set_ticks(ticks_log)         
        cbar.set_ticklabels(ticks_original)
        
        plt.tight_layout(rect=[0, 0, 0.9, 1]) 
        save_filename = f"{model_name.lower().replace('-', '_')}_len_ensemble_{agg_type}.pdf"
        save_path = os.path.join(save_dir, save_filename)
        fig.savefig(
            save_path, 
            bbox_inches='tight', 
        ...
        save_filename = f"{model_name.lower().replace('-', '_')}_len_ensemble_{agg_type}.png"
        save_path = os.path.join(save_dir, save_filename)
        fig.savefig(
            save_path, 
            dpi=400,
...
def plot_models_ensemble(
    ...
    for agg_type, points in plot_data.items():
        ...
        y_min = np.min(y_vals) * 0.94
        y_max = np.max(y_vals) * 1.01
        ax.set_ylim(y_min, y_max)
...
def main():
    ...
    parser.add_argument('--models', '-m', type=str, default="moirai-1.1-R-small")
    ...
    parser.add_argument('--disturbance', '-p', type=str, default="")
    ...
    print(f"save_path：{args.save_dir}")
